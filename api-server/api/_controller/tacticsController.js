...
const getTotal = async () => {
  ...
  try {
    const query = `SELECT COUNT(*) AS cnt FROM ${TABLE.TACTICS}`;
...
const getSelectOne = async (id) => {
  // const getTotal = async function () {
  try {
    const query = `SELECT COUNT(*) AS cnt FROM ${TABLE.TACTICS} WHERE id=?`;
    const values = [id];
...
const getList = async (req) => {
  try {
    ...
    const query = `SELECT * FROM ${TABLE.TACTICS} ${where} order by id desc limit 0, ${len}`;
...
const tacticsController = {
  ...
  create: async (req) => {
    ...
    try {
      const query = `INSERT INTO tactics (title, done) VALUES (?,?)`;
  ...
  update: async (req) => {
    ...
    try {
      const query = `UPDATE ${TABLE.TACTICS} SET title =?, done=? WHERE id= ?`;
  ...
  delete: async (req) => {
    ...
    try {
      ...
      const query = `DELETE FROM ${TABLE.TACTICS} WHERE id = ?;`;
      const values = [id];
      const [rows] = await db.execute(query, values);
      if (rows.affectedRows == 1) {
        return resData(
    ...
    reset: async (req) => {
      // truncate
      try {
        const query = `TRUNCATE TABLE ${TABLE.TACTICS};`;
      ...
      try {
        ...
        let query = `INSERT INTO tactics (title, done) VALUES `; //.values( data1 ), (data2),,,
        let arr = [];
        for (let i = 0; i < len; i++) {
...
module.exports = tacticsController;